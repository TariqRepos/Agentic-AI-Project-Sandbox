 ...
 * an asyncronous (nonblocking call) by streaming the output in chucks
 */
public class ClaudeChatWithStreaming {
    ...
    public static void main(String... args) throws Exception {
        ...
        // Create a latch that allows one thread to wait until a signal is received (count reaches zero)
        final CountDownLatch countDownLatch = new CountDownLatch(1);

        // Set up AWS Bedrock async client to interact with FMs using credentials from profile
        try (BedrockRuntimeAsyncClient bedrockClient = BedrockRuntimeAsyncClient.builder()
                .region(Region.US_EAST_1)
                .credentialsProvider(ProfileCredentialsProvider.create())
                .build()) {

            // Create a virtual thread executor to run async tasks without blocking main thread
            try (var executor = Executors.newVirtualThreadPerTaskExecutor()) {
                executor.submit(() -> {

                    // Create body for Bedrock requests with inference parameters 
                    String bedrockBody = BedrockRequestBody.builder()
                        .withModelId(MODEL_ID)
                        .withPrompt(PROMPT)
                        // Set the maximum number of tokens 
                        .withInferenceParameter("max_tokens_to_sample", 2048)
                        // Control the randomness/creativity of the output
                        .withInferenceParameter("temperature", 0.5)
                        // Limits the sampling to the top K most likely tokens at each generation step
                        .withInferenceParameter("top_k", 250)
                        // Uses all tokens; does not limit probability mass
                        .withInferenceParameter("top_p", 1)
                        .build();

                    // Create model specific streaming request using utlity class body
                    ...
                    // Call the Bedrock model asynchronously and stream the response
                    bedrockClient.invokeModelWithResponseStream(invokeModelRequest,
                            InvokeModelWithResponseStreamResponseHandler.builder()
                                    // Log response metadata received (status, headers, etc.)
                                    .onResponse(response -> {
                                        System.out.println("🤖 Response: ");
                                    })

                                    // Process the streamed response chunks from the model
                                    .subscriber(eventConsumer -> {
                                        eventConsumer.accept(new InvokeModelWithResponseStreamResponseHandler.Visitor() {
                                            public void visitChunk(PayloadPart payloadPart) {
                                                // Parse chunks for "completeion" for the Claude model
                                    ...
                                    // Mark the end of streaming and release the latch to continue main thread
                                    .onComplete(() -> {
                                        System.out.println();
                ...
                // Wait for the streamed response to complete before exiting the try block
